...
- **Full hydrator:** CSV keys are normalized (BOM stripped) so cleaned files with `\ufeffdatasheet_id` still match. For each CSV, looks in `data/wahapedia/Cleaned_CSVs/` first (same or alternate filename), then in `data/wahapedia/`, **except** for the core id-chain files `Datasheets.csv`, `Datasheets_models.csv`, `Datasheets_models_cost.csv`, which **always use root** so ids (e.g. `000000001`) stay in sync and points apply. Delimiter is auto-detected (pipe vs comma). Numeric ids from cleaned files are normalized to 9-digit form where needed.
